...
try{
    ...
    $link = new \PDO(   'mysql:host=localhost;dbname=amdbms;port=3309', 
                        ...
                        $month=$_POST['month'];
                        $handle = $link->prepare("select Grade_B_Price, day  from market_price where Branch_Id='$Branch_Id' and m_date between '2022/$month/1' and '2022/$month/31' order by day"); 
                        $handle->execute(); 
    ...
    foreach($result as $row){
        array_push($market_price, array("x"=> $row->day, "y"=> $row->Grade_B_Price));
    }
}
	$link = null;
}
